...
from batch.schwab_csv_ingestor import SchwabIngestor
from batch.schwab_csv_ingestor import RowParserException
from stock_analysis.logic import order_history


class TestSchwabCsvIngestor(object):

    def test_init(self):
        batch = SchwabIngestor()
    ...
    def test_run(self):
        ...
        with \
            ...
            assert patch_add_orders.call_args_list == [
                mock.call(batch.user_id, mock_parsed_orders)
            ]
    ...
    def test_parse_orders_from_csv_reader(self):
        ...
        reader = [
            '06/12/2017,Buy,NFLX,NETFLIX COM INC,19,$153.6799, ($2924.87)'.split(','),
            '06/08/2017,Buy,NFLX,NETFLIX COM INC,39,$151.9, ($2924.87)'.split(','),
        ]
    ...
    def test_extract_order_from_row_skips_malformed_row(self):
        batch = SchwabIngestor()
        row = 'MALFORMED_DATE,Buy,NFLX,NETFLIX COM INC,19,$153.6799, ($2924.87)'.split(',')
    ...
    def test_extract_order_from_row_ignores_non_bought_txn_type(self):
        batch = SchwabIngestor()
        row = '06/12/2017,UKNOWN_TXN_TYPE,NFLX,NETFLIX COM INC, 39, $151.9, ($2924.02)'.split(',')
